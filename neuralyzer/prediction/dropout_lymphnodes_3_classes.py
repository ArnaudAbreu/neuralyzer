...
from ..data.wsi2pred import predict_slides, dropout_predict_slides_from_labpathlist, dropout_predict_slides_from_dir
from ..model import CCLF
...
def test(labpathfile, device, basenet, outfolder, predlevel, patchsize, patchinter, inputchannels, sampling=100):
    ...
    clf = CCLF(refarchi, height=h, width=w, colors=c, n_classes=3, learning_rate=0.001, model_path=basenet, optimizer="SGD", sampling=sampling)
...
def test_dir(dirname, device, basenet, outfolder, predlevel, patchsize, patchinter, inputchannels, sampling=100):
    ...
    outputdir = os.path.join(outfolder, 'slides_prediction_' + os.path.basename(dirname))
    ...
    # predict_slides(clf, slidedir, outputdir, prediction_level=predlevel, n_classes=2)
    # predict_slides_from_dir(clf, slidedir, outputdir, predlevel, interval, w, h)
    dropout_predict_slides_from_dir(clf, dirname, outputdir, predlevel, patchinter, w, h)
