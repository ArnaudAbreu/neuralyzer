...
def get_median_slide_pred(prediction_file):
    ...
    return numpy.median(preds_m, axis=0), numpy.mean(preds_v, axis=0), gt
...
def get_majority_slide_pred(prediction_file):
    ...
    preds = numpy.asarray([get_stat_patch_pred(elem['prediction']) for elem in data])
    preds_v = numpy.asarray([p[1] for p in preds])
    preds_m = numpy.asarray([p[0] for p in preds])
    labpred = numpy.argmax(preds_m, axis=1)
    n_tiles = preds_m.shape[0]
    ...
    if data[0]['groundtruth'] == 'HF':
        ...
        gt = numpy.array([0, 1], int)
    return numpy.asarray([hfpercent, lfpercent]), numpy.mean(preds_v, axis=0), gt
...
for k in numpy.where(abstruth != abspreds)[0]:
    bp = dict()
    bp['name'] = names[k]
    bp['prediction'] = predictions[k]
    bp['groundtruth'] = groundtruths[k]
    bp['variance'] = vars[k]
    bp['certainty'] = 1. - (vars[k][0] / vars.max())
    badpredicted.append(bp)
    print('name: ', names[k], '\n',
          'prediction: ', predictions[k], '\n',
          'groundtruth: ', groundtruths[k], '\n',
          'variance: ', vars[k], '\n',
          'certainty: ', 1. - (vars[k][0] / vars.max()), '\n',
          '#' * 20)
...
for k in numpy.where(abstruth == abspreds)[0]:
    wp = dict()
    wp['name'] = names[k]
    wp['prediction'] = predictions[k]
    wp['groundtruth'] = groundtruths[k]
    wp['variance'] = vars[k]
    wp['certainty'] = 1. - (vars[k][0] / vars.max())
    wellpredicted.append(wp)
